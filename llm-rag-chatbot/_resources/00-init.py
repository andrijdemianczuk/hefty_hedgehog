...
# MAGIC <img width="1px" src="https://www.google-analytics.com/collect?v=1&gtm=GTM-NKQ8TT7&tid=UA-163989034-1&aip=1&t=event&ec=dbdemos&ea=VIEW&dp=%2F_dbdemos%2Fdata-science%2Fllm-rag-chatbot%2F_resources%2F00-init&cid=1444828305810485&uid=2955235463274469">
...
def wait_for_index_to_be_ready(index_name, vsc):
  for i in range(180):
    idx = vsc.get_index(index_name).describe()
    status = idx['index_status']['status']
    if "provisioning" in status.lower() :
      if i % 20 == 0: 
        print(f"Waiting for index to build, this can take a few min... {idx['index_status']['message']} - {status}")
      time.sleep(10)
    elif "offline" in status.lower() or "failed" in status.lower():
...
def wait_for_vs_catalog_to_be_ready(catalog, timeout_sec = 1800):
    for i in range(int(timeout_sec/10)):
        ...
        if c['catalog_status']['state'] != 'STATE_NOT_READY':
            if c['catalog_status']['state'] == 'STATE_UNKNOWN':
                if i > 60:
                    print(f"WARNING: unknown state after too long: {catalog} - something might be wrong: {c}")
                    return
            else:
